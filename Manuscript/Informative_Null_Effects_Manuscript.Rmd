...
When an equivalence test is statistically significant, it is possible that there is an true effect effect - it might just be smaller than what was deemed meaningful when the study was designed.
...
Because Bayesian estimates are infuenced by the choice of a prior, the 95% HDI of the posterior distribution does *not* include parameter values that are likely to be *true*.
Instead, the 95% HDI contains the parameter values that are deemed most *credible* (hence the term 'credible interval'), given the data and the model.
...
Using priors is an automatic way to perform this shrinkage.
If a lot of data is available, the amount of shrinkage is reduced and the prior has less influence on the statistical inference.
This can be seen as a beneficial property of the way Bayesian estimation handles shrinkage.
...
Researchers who only rely on null-hypothesis tests limit themselves in only asking the question whether the null-hypothesis can be rejected.
