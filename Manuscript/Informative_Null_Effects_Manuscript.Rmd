...
We provide a hypothetical example that is analyzed using four different methods, discuss how to interpretat the results (as well as possible misinterpretations), and briefly explain which inferential frameworks these different methods are based on. 
...
The general recommendations in this paper apply also in more complex models.]
...
Non-significant results simply indicate that the data are not surprising if we assume there were no true differences between the manipulations.
This might be because there is no difference between the two populations from which the two groups are sampled, in which case a non-significant effect is expected with a frequency of `r 1-sig.level`.
...
This requires an *a priori* power analyses (as done above) under which an alternative hypothesis is set up and the long-run Type II error rate is chosen (through the chosen level of power).
In applied practice, there has evolved a hybrid between the two paradigms of statistical testing [@Perezgonzalez2015], that is mostly incompatible with either theory of statistical testing.
For proper statistical inferences it is important to use the statistical methods in line with their theoretical basis.
In this section and the section on equivalence testing, we focus on the Neyman-Pearson approach of hypothesis testing and interpret the results of a statistical test as a dichotomous decision for which we have decided on long-run error rates.
...
Differences between the confidence interval reported above and the Bayesian HDI are to be expected.
...
An alternative way to investigate the practical equivalence using the information contained in the posterior distribution, e.g. @Greenwald1975 recommends to investigate the probability mass contained in the ROPE [s. @Greenwald1975, p. 18].
...
It is important to note, however, that priors have different purposes in the two applications:
...
Therefore, it is cautioned against the use of 'default' priors when calculating Bayes factors [see @Dienes2014], which are a compromise between general expectations about effect sizes and useful mathematical properties [e.g. @Rouder2009], but should only be chosen if they actually reflect a useful alternative model given the research question.
...
Thus, although the data support the null model over the alternative model specified by the prior, there is no good reason to conclude in favor of either model - at least if not either model is much more reasonable than the other *a priori* without respect to the data (this issue is extended on below).
...
Awareness of the issue is nevertheless important and should even more underline the need for conducting direct replication studies, meta-analyses and the use of adequate statistical tools.
...
Even when the traditional significance is not significant while the equivalent is statistically significant (and we would know that we have not committed any Type II or Type I error respectively), it is still possible that there is a true effect - it might just be smaller than what was deemed meaningful when the study was designed and the Smallest Effect Size of Interest was chosen.
